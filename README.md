...
Being readable is not a licence to build or redistribute this repository (see Licence below). What a
customer is promised is narrower, and is `DECISIONS.md` D31: their machines keep booting whatever
happens to us, because the image is already on them; and if Auros ceases operating, each customer
receives the build files for their own image — their recipe, this base Containerfile and the build
scripts — so they or anyone they hire can keep patching it. That is not a licence to our tooling and
not redistribution rights. It has not been executed end to end yet.
