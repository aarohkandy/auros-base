...
# enable_unit() above is system-scope: it looks for the unit in /usr/lib/systemd/system and nowhere
# else, so it cannot enable a `systemd --user` unit. This is the user-scope equivalent — moved here
# from 40-windows-feel.sh once 45-restore.sh needed it too — built on the same principle: verify the
# filesystem state rather than trust an exit code, because the whole point of enable_unit's fallback
# path is that offline systemctl is the part we are least sure of.
#
