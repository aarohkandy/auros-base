...
| the test wrapper wrote `/etc/plasmalogin.conf.d/00-auros-matrix-autologin.conf` but not `/etc/plasma-setup-done` | KDE plasma-setup's unit (`ConditionPathExists=!/etc/plasma-setup-done`, `Before=display-manager.service`) wrote `99-plasma-setup.conf` = `User=plasma-setup`; plasmalogin's later conf.d file wins, so seat0 went to the first-boot wizard (run 35616444839: `1 968 plasma-setup seat0`) and B7/B8/B12 had no `auros` session. The wizard deletes its 99 file when it starts, so the diagnostic never showed it. The wrapper now writes the marker; the shipping image does not |
